...
2. **Build a NEW claims pipeline in parallel** that reads the v2 feed and produces the
   claims fact — writing the candidate output to `CLAIMS_CANDIDATE.SILVER.FACT_CLAIM`
   (same table name, separate candidate database). Do **not** modify the running
   production pipelines or anything in `REPLICATED.SILVER`.
...
- Matillion project **Claims_Analytics**, environment **claims**.
